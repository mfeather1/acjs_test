...
function AnimCube(params) {
  ...
  var D2 = D*D;
  var ofs = 12200;  // set above move range: 24 * (modeChar.length+1) = 12120
  ...
  function init() {
    ...
    */
    ...
    if ("0" == param)
      moveCounter = 0;
    curInfoText = (move.length > 0 && move[0][0] >= ofs) ? 0 : -1;
    init2();
    if (demo)
      startAnimation(-1);
  } // init()
  ...
  function getMovePart(sequence, info, num) {
    ...
    var length = 0;
    var mv = [];
    var moveCodeString = "UDFBLRESMXYZxyz";
    if (yzAlt == true)
      moveCodeString = "UDFBLRESMXZYxzy";
  ...
  function arrayMovePos(move, realPos) {
    ...
    var rpos = 0;
    while (true) {
      while (pos < move.length && move[pos] >= ofs)
        pos++;
      if (rpos == realPos)
        break;
      if (pos < move.length) {
  ...
  function initInfoText(move) {
    if (move.length > 0 && move[0] >= ofs)
      curInfoText = move[0] - ofs;
    else
      curInfoText = -1;
  }
  ...
  function doMove(cube, move, start, length, reversed) {
    ...
    while (true) {
      ...
      if (move[position] >= ofs) {
        curInfoText = reversed ? -1 : move[position] - ofs;
      }
  ...
  */
    ...
    function callback() {
      ...
      if (innerLoopTop) {
        ...
        if (!cont) {
          ...
          else if (mv[movePos] >= ofs)
            curInfoText = moveDir > 0 ? mv[movePos] - ofs : -1;
          else
            spin = true;
          if (spin) {
            num = mv[movePos] % 4 + 1;
      ...
      if (!cont) {
        ...
        if (innerLoopBot) {
          ...
          if (moveDir > 0) {
            movePos++;
            if (movePos < mv.length && mv[movePos] >= ofs) {
              curInfoText = mv[movePos] - ofs;
              movePos++;
            }
            if (movePos == mv.length) {
  ...
  function init2() {
    ...
    initCube();
    paint();
    // use this to display randmoves converted to sign notation
  ...
  function randMoves(s, n) {
    ...
    // to display converted randmoves add the following line to end of init2 
    // console.log(moveTextFunc(move[0], 0, move[0].length));
    return randmoves;
  }
  ...
  function convertNotation(s) {
    for (var i=0; i < 401; i++) // can be increased to 900
      s = replaceMoves(s, i+100, ua[98+i], 0);
    for (var i=0; i < 90; i++)
      s = replaceMoves(s, i+10, ua[8+i], 0);
    for (var i=0; i < 8; i++)
  ...
  function initTurnSymbol() {
    for (var i=0; i < D/2-2; i++) {
      var n = i + 3;
      turnSymbol[0][i+6] = [n+'U', n+'D', n+'F', n+'B', n+'L', n+'R'];
    }
    if (D%2) {
      var esm = ["~E", "E", "S", "~S", "M", "~M"];
    ...
    // console.log(turnSymbol[0]);
  }

  var ua = [];  // unicode array
  var us = 130; // unicode starting number (0x82)

  function initUnicode() {
    for (var i=0; i < D/2-1; i++)
      ua[i] =  String.fromCharCode(us+i);
    // for (var i=0; i < D/2-1; i++)
    //   console.log(ua[i].charCodeAt()); // add .toString(16) for hex
  }
  ...
  function initModeChar() {
    initUnicode();
    modeChar = [ua[0], '', '', '', ''];
    for(var i=0; i <= Math.ceil(D/2)-3; i++)
      modeChar[i+5] = ua[i+1]; 
  ...
  function initEyeOrder() {
    var n = Math.ceil(D/2) + 4;
    for (var i=0; i < n; i++) {
      eyeOrder[i] = [];
      for (var j=0; j < D; j++)
         eyeOrder[i][j] = 0;
